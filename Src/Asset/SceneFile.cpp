...
void SceneFile::Load(const std::string& path, Scene& scene)
{
    ...
    if (first_time) {
        ConfigEntry* meta = info.GetEntry(HashStr64("Meta"));
        ...
        scene.Name = meta->GetMember(HashStr64("Name"))->Get<const char*>();
    }
    ...
    ConfigEntry* sun_entry = info.GetEntry(HashStr64("Sun"));

    if (sun_entry) {
        sun->SetPosition(sun_entry->GetMemberValue<Vec3f>(HashStr64("Pos"), Vec3f::sZero));

        sun->Color = sun_entry->GetMemberValue<Color>(HashStr64("Color"), Color::FromRGBA(100, 100, 100, 4));
        sun->AmbientColor = sun_entry->GetMemberValue<Color>(HashStr64("Color"), Color::FromRGBA(100, 100, 100, 1));
    }


    // Load objects

    ConfigEntry* object_list = info.GetEntry(HashStr64("Objects"));
...
{
    const char* mesh_path = object_entry.GetMember(HashStr64("Mesh"))->Get<const char*>();


    LoadObjectOptions load_options {};
    if (object_entry.GetMember(HashStr64("Physics")) != nullptr) {
...
{
    ConfigEntry* shadow_caster = object_entry.GetMember(HashStr64("Shadows"));
    if (shadow_caster != nullptr) {
        object->SetShadowCaster(static_cast<bool>(shadow_caster->Get<int64>()));
    }

    ConfigEntry* scale = object_entry.GetMember(HashStr64("Scale"));
    if (scale != nullptr) {
        object->SetScale(scale->Get<float32>());
    }

    object->SetPosition(object_entry.GetMemberValue(HashStr64("Pos"), object->mPosition));
    object->SetRotation(object_entry.GetMemberValue(HashStr64("Rot"), object->mRotation));
    object->SetScale(object_entry.GetMemberValue(HashStr64("Scale"), object->mScale));

    object->Update();
    object->MarkTransformOutOfDate();

    ConfigEntry* layer = object_entry.GetMember(HashStr64("Layer"));
    if (layer != nullptr) {
    ...
    ConfigEntry* unlit = object_entry.GetMember(HashStr64("Unlit"));
    if (unlit != nullptr) {
