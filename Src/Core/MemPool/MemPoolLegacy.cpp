...
// void MemPoolPage::Create(uint64 size)
// {
...
//     if (mMem || mCapacity) {
//         Panic("MemPool", "Memory pool has already been initialized!");
//     }

//     mCapacity = size;

//     void* allocated_ptr = std::malloc(mCapacity);
//     if (allocated_ptr == nullptr) {
//         Panic("MemPool", "Error allocating buffer for memory pool!");
//     }
...
//     LogDebug("Allocated {} mem block slots!", num_mem_blocks);
// }

// static inline uint8* GetAlignedPtr(uint8* ptr)
// {
//     return reinterpret_cast<uint8*>(AlignValue<scAlignToBytes>(reinterpret_cast<uintptr_t>(ptr)));
// }
...
//  */
// auto MemPoolPage::AllocateMemory(uint64 requested_size) -> MPLinkedList<MemPoolPage::MemBlock>::Node*
// {
//     // #ifdef FX_MEMPOOL_USE_ATOMIC_LOCKING
//     SpinThreadGuard guard(&mInUse);
//     // #endif
...
//     const uint64 aligned_size = AlignValue<scAlignToBytes>(requested_size);

//     // If there are no allocations currently, allocate at start and exit early
//     if (!mMemBlocks.Tail) {
//         // Create a new block entry for the allocation
//         MemPoolPage::MemBlock new_block { .Size = aligned_size, .Start = GetAlignedPtr(new_block_ptr) };
...
//     const MemPoolPage::MemBlock& last_block = mMemBlocks.Tail->Data;
...
//     if (bytes_used_in_page < AlignValue<scAlignToBytes>(mCapacity)) {
//         // Create a new block entry for the allocation
//         MemPoolPage::MemBlock new_block { .Size = aligned_size, .Start = GetAlignedPtr(new_block_ptr) };
...
//         // at the very end of the gap. Since we already have this allocation, this will save us from having to
//         iterate
...
//             MemPoolPage::MemBlock new_block { .Size = aligned_size, .Start = node->Data.Start - aligned_size };
...
//     LogWarning("Using slow allocation path in MemPool!");
// #endif
...
//     while (node != nullptr) {
//         MemPoolPage::MemBlock& block = node->Data;
...
//         MemPoolPage::MemBlock& next_block = node->Next->Data;
...
//             MemPoolPage::MemBlock new_block { .Size = aligned_size, .Start = GetAlignedPtr(current_block_end) };
...
// auto MemPoolPage::GetNodeFromPtr(void* ptr) const -> MPLinkedList<MemPoolPage::MemBlock>::Node*
// {
//     SpinThreadGuard guard(&mInUse);
...
// MemPoolLegacy& MemPoolLegacy::GetGlobalPool()
// {
//     static MemPoolLegacy global_pool;
//     return global_pool;
// }

// #ifdef FX_MEMPOOL_TRACK_STATISTICS
// #define ALLOC_UPDATE_STATS \
//     if (mBytesInUse > mStats.BytesAllocatedPeak) { \
//         mStats.BytesAllocatedPeak = mBytesInUse; \
//     } \
//     ++mStats.TotalAllocs;
// #else
// #define ALLOC_UPDATE_STATS
...
// #define ALLOC_RETURN_NEW_ALLOCATION \
//     mBytesInUse += node->Data.Size; \
//     ALLOC_UPDATE_STATS; \ return node;


// auto MemPoolLegacy::AllocateMemory(uint64 requested_size) -> MPLinkedList<MemPoolPage::MemBlock>::Node*
// {
//     SpinThreadGuard guard(&mbInUse);
...
//     // If our node was successfully allocated, return it!
//     if (node != nullptr) {
//         ALLOC_RETURN_NEW_ALLOCATION;
//     }
...
//     if (node == nullptr) {
//         printf("Allocating new MemPool page!\n");
//         AllocateNewPage();
...
//     if (node == nullptr) {
//         Panic("MemPool", "Allocation is too large for memory pool page size!");
//     }
...
// void* MemPoolPage::Realloc(void* ptr, uint32 new_size)
// {
//     auto* node = GetNodeFromPtr(ptr);

//     // Adjust the count of bytes allocated
//     // int64 change_in_size = static_cast<int64>(new_size) - node->Data.Size;

//     MemPoolPage::MemBlock& block = node->Data;

//     // Node is at end of block list, simply increase size
//     if (node->Next == nullptr) {
//         node->Data.Size = AlignValue<scAlignToBytes>(new_size);
...
// void MemPoolPage::PrintAllocations() const
// {
//     auto* node = mMemBlocks.Head;

//     LogDebug("");
//     LogDebug("=== MemStat [size={:d}, start={:p}, end={:p}, freed={:d}] ===", mCapacity,
//                reinterpret_cast<void*>(mMem), reinterpret_cast<void*>(mMem + mCapacity), mFreedTotalSize);
...
//             if (gap_size) {
//                 LogDebug("MemGap  [size={:d}, ptr={:p}]", gap_size, reinterpret_cast<void*>(current_block_end));
//             }
//         }
...
//             if (gap_size) {
//                 LogDebug("MemGap  [size={:d}, ptr={:p}]", gap_size, reinterpret_cast<void*>(mMem));
//             }
//         }

//         LogDebug("MemAlloc[size={:d}, ptr={:p}]", block.Size, reinterpret_cast<void*>(block.Start));
...
// void MemPoolLegacy::Create(uint64 page_size, uint64 size_unit)
// {
//     SpinThreadGuard guard(&mbInUse);
//     {
//         mPageCapacity = page_size * size_unit;
...
// void MemPoolLegacy::PrintAllocations()
// {
//     MemPoolLegacy& pool = GetGlobalPool();
...
// void* MemPoolLegacy::AllocRaw(uint32 size, MemPoolLegacy* pool)
// {
//     if (pool == nullptr) {
//         pool = &GetGlobalPool();
//     }
...
//     MemPoolPage::MemBlock& block = pool->AllocateMemory(size)->Data;

//     return static_cast<void*>(block.Start);
// }

// bool MemPoolLegacy::IsPtrInPage(void* ptr, MemPoolPage* page) const
// {
//     const uint8* start_ptr = page->mMem;
//     const uint8* end_ptr = start_ptr + mPageCapacity;
...
// MemPoolPage* MemPoolLegacy::FindPtrInPage(void* ptr)
// {
...
//     if (num_pages == 0) {
//         LogError("Cannot find ptr in page when there are no pages in memory pool!");
//     }
...
// void MemPoolLegacy::AllocateNewPage()
// {
//     MemPoolPage* page = mPoolPages.Insert();

//     mpCurrentPage = page;
//     mpCurrentPage->Create(mPageCapacity);

//     LogInfo("Allocating a new memory page!");
// }

// void MemPoolLegacy::FreeRaw(void* ptr, MemPoolLegacy* pool)
// {
//     if (pool == nullptr) {
//         pool = &GetGlobalPool();
//     }
...
//         LogWarning("Attempting to free a ptr {:p} after mempool is destroyed", ptr);
//         FX_BREAKPOINT;
// #endif
//         return;
//     }
...
//     MemPoolPage* page = pool->FindPtrInPage(ptr);

//     if (page == nullptr) {
//         LogError("MemPool::Free: Pointer {:p} not found! Has it been freed already?", ptr);
...
//     const uint32 bytes_freed = page->Free(ptr);
//     pool->mBytesInUse -= bytes_freed;
// }
