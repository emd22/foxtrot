...
uint32 String::Find(const char* other) const
{
	const char* ptr = GetInternalPtr();

	// Truth be told, there is no way to beat the stdlib in speed without writing an SSE implementation of this. And
	// thats pretty overkill for what we need it for.
	const char* result = std::strstr(ptr, other);
