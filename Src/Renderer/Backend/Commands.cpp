...
// #define FX_ENFORCE_COMMAND_STATE 1
...
#ifdef FX_ENFORCE_COMMAND_STATE
	State = eCommandState::Reset;
#endif
...
{
#ifdef FX_ENFORCE_COMMAND_STATE
	Assert(State == eCommandState::Reset);
	State = eCommandState::InProgress;
#endif
...
{
#ifdef FX_ENFORCE_COMMAND_STATE
	if (State == eCommandState::Reset) {
		return;
	}

	Assert(State == eCommandState::Ended);
	State = eCommandState::Reset;
#endif
...
{
#ifdef FX_ENFORCE_COMMAND_STATE
	Assert(State == eCommandState::InProgress);
	State = eCommandState::Ended;
#endif
