...
Token &FxConfigScript::EatToken(TT token_type) {
  ...
  if (token.Type != token_type) {
    printf("[ERROR] %u:%u: Unexpected token type %s when expecting %s!\n",
           ...
           FxTokenizer::GetTypeName(token.Type),
...
void PrintDocCommentExample(FxTokenizer::Token *comment, int example_tag_length,
...
void PrintDocComment(FxTokenizer::Token *comment, bool is_command_mode) {
  ...
  if (comment->Length > example_tag_length &&
      !strncmp(start, example_tag, example_tag_length)) {
    printf("\n\t-> Script : ");
...
bool FxConfigScript::ExecuteUserCommand(const char *command,
...
FxAstNode *FxConfigScript::ParseRhs() {
  ...
  if (mTokenIndex + 1 < mTokens.Size()) {
    ...
    if (mInCommandMode) {
      has_parameters =
          next_token_type == TT::Identifier || next_token_type == TT::Integer ||
  ...
  if (token.Type == TT::Identifier) {
    if (has_parameters) {
      ...
      FxScriptExternalFunc *external_action =
          FindExternalAction(token.GetHash());
...
void FxConfigScript::DefineExternalVar(const char *type, const char *name,
                                       const FxScriptValue &value) {
...
FxAstNode *FxConfigScript::ParseStatementAsCommand() {
  ...
  if (mTokenIndex < mTokens.Size() && GetToken().Type == TT::Identifier) {
    ...
    // If there is what looks to be a function call, try it
      ...
      FxScriptExternalFunc *external_action =
          FindExternalAction(GetToken().GetHash());
      if (external_action != nullptr) {
...
FxAstNode *FxConfigScript::ParseStatement() {
  ...
  if (!node &&
      (mTokenIndex < mTokens.Size() && GetToken().Type == TT::Identifier)) {
    if (mTokenIndex + 1 < mTokens.Size() && GetToken(1).Type == TT::LParen) {
      node = ParseActionCall();
    } else if (mTokenIndex + 1 < mTokens.Size() &&
               GetToken(1).Type == TT::Equals) {
      Token &assign_name = EatToken(TT::Identifier);
      node = TryParseAssignment(&assign_name);
    } else {
      GetToken().Print();
...
FxScriptExternalFunc *
FxScriptInterpreter::FindExternalAction(FxHash hashed_name) {
  for (FxScriptExternalFunc &func : mExternalFuncs) {
    if (func.HashedName == hashed_name) {
      return &func;
    }
...
bool FxScriptInterpreter::CheckExternalCallArgs(FxAstActionCall *call,
...
FxScriptValue
FxScriptInterpreter::VisitExternalCall(FxAstActionCall *call,
                                       FxScriptExternalFunc &func) {
  FxScriptValue return_value;
...
FxScriptValue FxScriptInterpreter::VisitActionCall(FxAstActionCall *call) {
  ...
  std::vector<FxAstNode *> param_decls =
      call->Action->Declaration->Params->Statements;
...
void FxConfigScript::DefineDefaultExternalFunctions() {
  ...
  RegisterExternalFunc(
      ...
      [](FxScriptVM *vm, std::vector<FxScriptValue> &args,
         FxScriptValue *return_value) {
        printf("[SCRIPT]: ");
  ...
  //                    FxAstVarDecl* param_decl =
  //                    reinterpret_cast<FxAstVarDecl*>(param_node);
...
void FxScriptInterpreter::Visit(FxAstNode *node) {
  ...
  if (node->NodeType == FX_AST_BLOCK) {
    ...
    FxScriptAction action(actiondecl->Name, mCurrentScope, actiondecl->Block,
                          actiondecl);
    mCurrentScope->Actions.Insert(action);
...
void FxScriptInterpreter::DefineExternalVar(const char *type, const char *name,
                                            const FxScriptValue &value) {
  Token *name_token = FX_SCRIPT_ALLOC_MEMORY(Token, sizeof(Token));
  Token *type_token = FX_SCRIPT_ALLOC_MEMORY(Token, sizeof(Token));
...
const FxScriptValue &
FxScriptInterpreter::GetImmediateValue(const FxScriptValue &value) {
...
void FxConfigScript::RegisterExternalFunc(
    FxHash func_name, std::vector<FxScriptValue::ValueType> param_types,
...
FxScriptBCEmitter::FindVarHandle(FxHash hashed_name) {
...
FxScriptBytecodeActionHandle *
FxScriptBCEmitter::FindActionHandle(FxHash hashed_name) {
...
FxScriptRegister
FxScriptBCEmitter::RegFlagToReg(FxScriptRegisterFlag reg_flag) {
  switch (reg_flag) {
...
void FxScriptBCEmitter::MarkRegisterUsed(FxScriptRegister reg) {
  ...
  mRegsInUse =
      static_cast<FxScriptRegisterFlag>(uint16(mRegsInUse) | uint16(rflag));
}
...
void FxScriptBCEmitter::EmitSaveAbsoluteReg32(uint32 position,
...
void FxScriptBCEmitter::EmitLoadAbsolute32(uint32 position,
                                           FxScriptRegister output_reg) {
  // LOADA [i32] [%r32]
...
FxScriptRegister
FxScriptBCEmitter::EmitBinop(FxAstBinop *binop,
                             FxScriptBytecodeVarHandle *handle) {
  bool will_preserve_lhs = false;
  // Load the A and B values into the registers
  FxScriptRegister a_reg =
      EmitRhs(binop->Left, RhsMode::RHS_FETCH_TO_REGISTER, handle);
  ...
  FxScriptRegister b_reg =
      EmitRhs(binop->Right, RhsMode::RHS_FETCH_TO_REGISTER, handle);
...
FxScriptRegister FxScriptBCEmitter::EmitVarFetch(FxAstVarRef *ref,
                                                 RhsMode mode) {
...
void FxScriptBCEmitter::DoLoad(uint32 stack_offset, FxScriptRegister output_reg,
                               bool force_absolute) {
  if (stack_offset < 0xFFFE && !force_absolute) {
    ...
    int input_offset =
        -(static_cast<int>(mStackOffset) - static_cast<int>(stack_offset));
...
void FxScriptBCEmitter::DoSaveInt32(uint32 stack_offset, uint32 value,
                                    bool force_absolute) {
  if (stack_offset < 0xFFFE && !force_absolute) {
    // Relative save
...
void FxScriptBCEmitter::DoSaveReg32(uint32 stack_offset, FxScriptRegister reg,
                                    bool force_absolute) {
  if (stack_offset < 0xFFFE && !force_absolute) {
    // Relative save
...
void FxScriptBCEmitter::EmitAssign(FxAstAssign *assign) {
  FxScriptBytecodeVarHandle *var_handle =
      FindVarHandle(assign->Var->Name->GetHash());
  if (var_handle == nullptr) {
    printf("!!! Var '%.*s' does not exist!\n", assign->Var->Name->Length,
           assign->Var->Name->Start);
    return;
  }
...
FxScriptRegister
FxScriptBCEmitter::EmitLiteralInt(FxAstLiteral *literal, RhsMode mode,
                                  FxScriptBytecodeVarHandle *handle) {
...
FxScriptRegister
FxScriptBCEmitter::EmitLiteralString(FxAstLiteral *literal, RhsMode mode,
                                     FxScriptBytecodeVarHandle *handle) {
  const uint32 string_length = strlen(literal->Value.ValueString);

  // Emit the length and string data
  const uint32 string_position =
      EmitDataString(literal->Value.ValueString, string_length);
...
FxScriptRegister FxScriptBCEmitter::EmitRhs(FxAstNode *rhs,
                                            FxScriptBCEmitter::RhsMode mode,
                                            FxScriptBytecodeVarHandle *handle) {
  if (rhs->NodeType == FX_AST_LITERAL) {
    ...
    if (literal->Value.Type == FxScriptValue::INT) {
      ...
      FxScriptRegister output_register =
          EmitVarFetch(literal->Value.ValueRef, mode);
...
FxScriptBytecodeVarHandle *
FxScriptBCEmitter::DoVarDeclare(FxAstVarDecl *decl, VarDeclareMode mode) {
  ...
  FxScriptBytecodeVarHandle *inserted_handle =
      &VarHandles[VarHandles.Size() - 1];
...
FxScriptBytecodeVarHandle *
FxScriptBCEmitter::DefineAndFetchParam(FxAstNode *param_decl_node) {
  ...
  FxScriptBytecodeVarHandle *handle = DoVarDeclare(
      reinterpret_cast<FxAstVarDecl *>(param_decl_node), DECLARE_NO_EMIT);
...
FxScriptBytecodeVarHandle *
FxScriptBCEmitter::DefineReturnVar(FxAstVarDecl *decl) {
  RETURN_VALUE_IF_NO_NODE(decl, nullptr);
...
void FxScriptBCEmitter::EmitAction(FxAstActionDecl *action) {
  ...
  const uint16 distance_to_action =
      static_cast<uint16>(end_of_action - (start_of_action)-4);

  printf("End of action: %zu\n", end_of_action);
  printf("Distance to action: %d\n", distance_to_action);

  // Update the jump to the end of the action
  mBytecode[header_jump_start_index] =
      static_cast<uint8>(distance_to_action >> 8);
  mBytecode[header_jump_start_index + 1] =
      static_cast<uint8>((distance_to_action & 0xFF));
  ...
  const size_t number_of_scope_var_handles =
      VarHandles.Size() - start_var_handle_count;
...
void FxScriptBCPrinter::DoLoad(char *s, uint8 op_base, uint8 op_spec_raw) {
  ...
  if (op_spec == OpSpecLoad_Int32) {
    ...
    BC_PRINT_OP("load32 %d, %s", offset,
                FxScriptBCEmitter::GetRegisterName(
                    static_cast<FxScriptRegister>(op_reg)));
...
void FxScriptBCPrinter::DoPush(char *s, uint8 op_base, uint8 op_spec) {
  if (op_spec == OpSpecPush_Int32) {
    ...
    uint16 reg = Read16();
    BC_PRINT_OP("push32r %s", FxScriptBCEmitter::GetRegisterName(
                                  static_cast<FxScriptRegister>(reg)));
  }
}
...
void FxScriptBCPrinter::DoPop(char *s, uint8 op_base, uint8 op_spec_raw) {
  ...
  if (op_spec == OpSpecPush_Int32) {
    BC_PRINT_OP("pop32 %s", FxScriptBCEmitter::GetRegisterName(
                                static_cast<FxScriptRegister>(op_reg)));
  }
}
...
void FxScriptBCPrinter::DoArith(char *s, uint8 op_base, uint8 op_spec) {
  ...
  if (op_spec == OpSpecArith_Add) {
    BC_PRINT_OP("add32 %s, %s",
                FxScriptBCEmitter::GetRegisterName(
                    static_cast<FxScriptRegister>(a_reg)),
                FxScriptBCEmitter::GetRegisterName(
                    static_cast<FxScriptRegister>(b_reg)));
  }
}
...
void FxScriptBCPrinter::DoSave(char *s, uint8 op_base, uint8 op_spec) {
  ...
  // Save a register into an offset in the stack
    ...
    BC_PRINT_OP(
        "save32r %d, %s", offset,
        FxScriptBCEmitter::GetRegisterName(static_cast<FxScriptRegister>(reg)));
...
void FxScriptBCPrinter::DoJump(char *s, uint8 op_base, uint8 op_spec) {
  if (op_spec == OpSpecJump_Relative) {
    ...
    uint16 reg = Read16();
    BC_PRINT_OP("jmpar %s", FxScriptBCEmitter::GetRegisterName(
                                static_cast<FxScriptRegister>(reg)));
...
void FxScriptVM::PrintRegisters() {
  ...
  printf("X0=%u\tX1=%u\tX2=%u\tX3=%u\n", Registers[FX_REG_X0],
         Registers[FX_REG_X1], Registers[FX_REG_X2], Registers[FX_REG_X3]);
...
void FxScriptVM::DoSave(uint8 op_base, uint8 op_spec) {
  ...
  if (op_spec == OpSpecSave_AbsoluteInt32 ||
      op_spec == OpSpecSave_AbsoluteReg32) {
    offset = Read32();
  } else {
